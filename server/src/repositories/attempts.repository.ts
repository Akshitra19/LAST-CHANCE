...
export async function findAnswerScoringState(attemptId: string): Promise<Array<{ question_id: string; submitted_answer: Json; is_correct: boolean | null; marks_awarded: number | null; time_seconds: number; marked_for_review: boolean }>> { const { data, error } = await db().from('answers').select('question_id,submitted_answer,is_correct,marks_awarded,time_seconds,marked_for_review').eq('attempt_id', attemptId); if (error) failed(); return data; }
