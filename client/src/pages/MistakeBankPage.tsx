...
export function MistakeBankPage() {
  ...
  useUnsavedChanges('mistake-retry', dialogOpen);
  return <section className="mistakes-page" aria-labelledby="mistakes-title">
    ...
    <DelayedPageSkeleton cards={3} label="Loading mistakes" pending={state==='loading'}/>
    {state === 'error' && <div className="panel-state" role="alert"><p>Couldn't load Mistake Bank. Saved classifications were not changed.</p><button className="retry-button" onClick={() => void load()} type="button">Retry</button></div>}
    {state === 'ready' && result?.items.length === 0 && <div className="results-empty"><h2>{filtered ? 'No mistakes match these filters.' : 'Your Mistake Bank is clear.'}</h2><p>Wrong and skipped questions will appear here after tests.</p></div>}
