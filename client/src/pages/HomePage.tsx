...
export function HomePage() {
  ...
  const targetHours = settings ? studyTargetHours(settings, tasksState === 'ready' ? tasks.map((task) => task.plannedMinutes) : []) : null;
  ...
  return <section className="dashboard dashboard--command" aria-labelledby="home-title">
    ...
    <div className="dashboard-grid">
      ...
      <section className="dashboard-card" aria-labelledby="target-card-title"><h2 id="target-card-title">Daily target</h2>{settingsState === 'loading'&&<InlineSpinner label="Loading study target"/>}{settings&&settingsState==='ready'&&targetHours!==null&&<><p className="metric">{targetHours} <span>{targetHours===1?'hour':'hours'}</span></p><p className="dashboard-card__detail">{tasksState==='ready'&&tasks.length>0?'Based on today’s generated tasks.':`Target score: ${settings.targetMarks}/100`}</p></>}{settingsState==='error'&&<p className="card-state">Unavailable independently of your plan.</p>}</section>
      <section className="dashboard-card dashboard-card--syllabus" aria-labelledby="status-card-title"><div className="card-heading"><div><h2 id="status-card-title">Syllabus pulse</h2>{summary&&<p>{summary.total} actionable leaves · {syllabus?.officialTopicCount} official items</p>}</div><Link className="text-link" to="/syllabus">Open syllabus</Link></div>{syllabusState==='loading'&&<InlineSpinner label="Loading syllabus progress"/>}{syllabusState==='error'&&<CardError label="Couldn’t load syllabus progress." onRetry={loadSyllabus}/>} {syllabusState==='ready'&&summary&&<><div className="syllabus-pulse"><strong>{summary.counts.MASTERED} / {summary.total}</strong><span>actionable leaves mastered</span><div className="progress-track"><span style={{ width: `${Math.round(summary.counts.MASTERED / Math.max(1, summary.total) * 100)}%` }}/></div></div><dl className="status-grid status-grid--compact">{displayedStatuses.map((status)=><div key={status}><dt>{topicStatusLabels[status]}</dt><dd data-status={status}>{summary.counts[status]}</dd></div>)}</dl></>}</section>
    </div>
