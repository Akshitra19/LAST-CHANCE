...
function validate(values: FormValues): Record<string, string> {
  ...
  check('targetMarks', 100, 'Target marks'); check('weekdayStudyHours', 24, 'Study hours'); check('sundayStudyHours', 24, 'Study hours'); check('mondayStudyHours', 24, 'Study hours');
  return errors;
}
...
export function SettingsPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const next = { examDate: values.examDate || null, targetMarks: Number(values.targetMarks), weekdayStudyHours: Number(values.weekdayStudyHours), sundayStudyHours: Number(values.sundayStudyHours), mondayStudyHours: Number(values.mondayStudyHours) };
    const original = { examDate: settings.examDate, targetMarks: settings.targetMarks, weekdayStudyHours: settings.weekdayStudyHours, sundayStudyHours: settings.sundayStudyHours, mondayStudyHours: settings.mondayStudyHours };
