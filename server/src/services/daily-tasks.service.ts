...
async function validateReferences(subjectId: string | null | undefined, topicId: string | null | undefined): Promise<void> {
  if (topicId && !subjectId) throw new AppError(400, 'TOPIC_REQUIRES_SUBJECT', 'Select a subject before selecting a topic.');
...
let timerStartQueue: Promise<void> = Promise.resolve();
async function startTimerExclusively(id: string) { const task = await findTaskRow(id); if (!task) throw new AppError(404, 'TASK_NOT_FOUND', 'Task was not found.'); if (task.started_at) throw new AppError(409, 'TIMER_ALREADY_RUNNING', 'This timer is already running.'); if (task.status !== 'TODO') throw new AppError(409, 'TASK_NOT_TODO', 'Reopen the task before starting its timer.'); const active = await findActiveTaskRow(); if (active) throw new AppError(409, 'ACTIVE_TIMER_EXISTS', 'Stop the current timer before starting another task.'); const now = new Date().toISOString(); const saved = await updateTaskRow(id, { started_at: now, updated_at: now }); if (!saved) throw new AppError(404, 'TASK_NOT_FOUND', 'Task was not found.'); return canonical(saved); }
export function startTimer(id: string) { const attempt = timerStartQueue.then(() => startTimerExclusively(id)); timerStartQueue = attempt.then(() => undefined, () => undefined); return attempt; }
export async function stopTimer(id: string) { const task = await findTaskRow(id); if (!task) throw new AppError(404, 'TASK_NOT_FOUND', 'Task was not found.'); if (!task.started_at) throw new AppError(409, 'TIMER_NOT_RUNNING', 'This timer is not running.'); const now = new Date(); let actual: number; try { actual = accumulatedActualMinutes(task.actual_minutes, new Date(task.started_at), now); } catch (error) { if (error instanceof AppError && error.code === 'TIMER_LIMIT_EXCEEDED') { await updateTaskRow(id, { started_at: null, updated_at: now.toISOString() }); } throw error; } const saved = await updateTaskRow(id, { actual_minutes: actual, started_at: null, updated_at: now.toISOString() }); if (!saved) throw new AppError(404, 'TASK_NOT_FOUND', 'Task was not found.'); return canonical(saved); }
