...
export function ResultsPage() {
  ...
  return <section className="results-page" aria-labelledby="results-title">
    ...
    {state === 'loading' && <p className="panel-state" role="status">Loading results…</p>}
    {state === 'error' && <div className="panel-state" role="alert"><p>Couldn't load results.</p><button className="retry-button" onClick={() => void load()} type="button">Retry</button></div>}
    ...
    {state === 'ready' && result && result.items.length > 0 && <div className="result-history">{result.items.map((item) => <article className="result-history-card" key={item.attemptId}>
