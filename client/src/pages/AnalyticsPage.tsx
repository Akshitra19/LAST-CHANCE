...
export function AnalyticsPage() {
  ...
  const load = useCallback(async (signal?: AbortSignal) => { setState('loading'); setData(null); try { setData(await getAnalytics({ range, testType }, signal)); setState('ready'); } catch { if (!signal?.aborted) setState('error'); } }, [range, testType]);
  ...
  return <section aria-labelledby="analytics-title" className="analytics-page">
    ...
    {state === 'loading' && <div aria-live="polite" className="analytics-state" role="status"><span className="analytics-spinner" aria-hidden="true"/>Loading analytics…</div>}
