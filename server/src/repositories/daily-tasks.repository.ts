...
export type TopicReference = Pick<Tables<'topics'>, 'id' | 'subject_id' | 'parent_topic_id' | 'code' | 'name'>;
const columns = 'id,task_date,subject_id,topic_id,task_type,planned_minutes,actual_minutes,status,started_at,completed_at,notes,plan_key,plan_slot,created_at,updated_at';
...
export async function listTaskRows(date: string): Promise<DailyTaskRow[]> { const { data, error } = await db().from('daily_tasks').select(columns).eq('task_date', date).order('plan_slot', { ascending: true, nullsFirst: false }).order('created_at').order('id'); if (error) failed(); return data; }
...
export async function findTopic(id: string): Promise<TopicReference | null> { const { data, error } = await db().from('topics').select('id,subject_id,parent_topic_id,code,name').eq('id', id).maybeSingle(); if (error) failed(); return data; }
...
export async function fetchTopicReferences(ids: string[]): Promise<TopicReference[]> { if (!ids.length) return []; const { data, error } = await db().from('topics').select('id,subject_id,parent_topic_id,code,name').in('id', ids); if (error) failed(); return data; }
