...
export function AttemptPage(){
  ...
  if(attempt.status==='SUBMITTED')return <main className="attempt-shell attempt-shell--submitted"><m.article animate={{opacity:1,scale:1}} className="submitted-card submitted-card--confirmed" initial={{opacity:0,scale:.97}} transition={{duration:motionTokens.reveal,ease:motionTokens.ease}}><span aria-hidden="true" className="submitted-mark">✓</span><p className="eyebrow">ATTEMPT SUBMITTED</p><h1>{attempt.test.name}</h1><p>Your responses are saved and this attempt is now immutable.</p><dl><div><dt>Answered</dt><dd>{answeredCount} of {attempt.questions.length}</dd></div><div><dt>Time used</dt><dd>{formatClock(attempt.totalTimeSeconds??0)}</dd></div></dl><div className="submitted-card__actions"><button className="primary-button" onClick={()=>navigate(`/results/${attempt.id}`)} type="button">View Results</button><button className="secondary-button" onClick={()=>navigate(`/tests/${attempt.test.id}/start`)} type="button">Retake Test</button><button className="secondary-button" onClick={()=>navigate('/test')} type="button">Back to Tests</button></div></m.article></main>;
  ...
  return <main className="attempt-shell">
    <header className="attempt-header"><button className="attempt-exit" onClick={()=>setExitOpen(true)} type="button">Exit</button><div><strong>{attempt.test.name}</strong><span>Question {currentIndex+1} of {attempt.questions.length}</span></div><time aria-label={`${remaining} seconds remaining`} className={`attempt-timer${remaining<=60?' attempt-timer--critical':remaining<=300?' attempt-timer--warning':''}`}>{formatClock(remaining)}</time></header>
    <p className={importantNotice?'attempt-notice':'sr-only'} aria-live="polite">{notice}</p>
    <div className="attempt-workspace">
      <m.section animate={{opacity:1,x:0}} className="attempt-question" initial={{opacity:0,x:4}} key={current.id} transition={{duration:motionTokens.micro,ease:motionTokens.ease}} aria-labelledby="attempt-question-title">
      ...
      <QuestionPalette attempt={attempt} currentIndex={currentIndex} mobileOpen={paletteOpen} onClose={closePalette} onGo={(index)=>void go(index)} onSubmit={()=>setSubmitOpen(true)}/>
    </div>
    <Dialog busy={submitting} onClose={()=>setExitOpen(false)} open={exitOpen} titleId="exit-title"><h2 id="exit-title">Exit this test?</h2><p>Your saved answers remain, but the timer continues and does not pause.</p><div><button autoFocus className="secondary-button" onClick={()=>setExitOpen(false)} type="button">Stay</button><button className="primary-button" onClick={()=>void exit()} type="button">Exit to Tests</button></div></Dialog>
    <Dialog busy={submitting} onClose={()=>setSubmitOpen(false)} open={submitOpen} titleId="submit-title"><h2 id="submit-title">Submit this attempt?</h2><p>{answeredCount} answered · {attempt.questions.length-answeredCount} unanswered · {reviewCount} marked for review.</p><p>Submission is final. You cannot change answers afterward.</p><div><button autoFocus className="secondary-button" onClick={()=>setSubmitOpen(false)} type="button">Keep working</button><button className="primary-button" disabled={submitting} onClick={()=>void completeSubmit()} type="button">{submitting?'Submitting…':'Confirm Submit'}</button></div></Dialog>
  </main>;
}
...
function QuestionPalette({attempt,currentIndex,mobileOpen,onClose,onGo,onSubmit}:{attempt:TestAttempt;currentIndex:number;mobileOpen:boolean;onClose:()=>void;onGo:(index:number)=>void;onSubmit:()=>void}){
  const drawerRef=useRef<HTMLElement>(null),returnRef=useRef<HTMLElement|null>(null);
  useEffect(()=>{if(!mobileOpen||!window.matchMedia('(max-width: 900px)').matches)return;returnRef.current=document.activeElement as HTMLElement|null;const prior=document.body.style.overflow;document.body.style.overflow='hidden';drawerRef.current?.querySelector<HTMLElement>('.palette-close')?.focus();const onKey=(event:KeyboardEvent)=>{if(event.key==='Escape'){event.preventDefault();onClose();return}if(event.key!=='Tab'||!drawerRef.current)return;const nodes=[...drawerRef.current.querySelectorAll<HTMLElement>('button:not([disabled])')];const first=nodes[0],last=nodes[nodes.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last?.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first?.focus()}};document.addEventListener('keydown',onKey);return()=>{document.body.style.overflow=prior;document.removeEventListener('keydown',onKey);returnRef.current?.focus()}},[mobileOpen,onClose]);
  return <aside aria-label="Question palette" aria-modal={mobileOpen||undefined} className={mobileOpen?'question-palette question-palette--open':'question-palette'} ref={drawerRef} role={mobileOpen?'dialog':undefined}><div className="question-palette__heading"><div><h2>Questions</h2><p>{attempt.questions.filter(answered).length} of {attempt.questions.length} answered</p></div><button aria-label="Close question palette" className="palette-close" onClick={onClose} type="button">×</button></div><div className="question-palette__grid">{attempt.questions.map((question,index)=>{const isAnswered=answered(question),state=question.markedForReview?(isAnswered?'Answered + Review':'Review'):(isAnswered?'Answered':'Not Answered');return <button aria-current={index===currentIndex?'step':undefined} aria-label={`Question ${index+1}: ${state}${index===currentIndex?', current':''}`} className={`palette-question palette-question--${state.toLowerCase().replaceAll(' ','-').replace('+','plus')}${index===currentIndex?' palette-question--current':''}`} key={question.id} onClick={()=>onGo(index)} type="button"><span>{index+1}</span><small>{state}{index===currentIndex?' · Current':''}</small></button>})}</div><div className="palette-legend"><span>○ Not Answered</span><span>● Answered</span><span>◆ Review</span><span>★ Answered + Review</span></div><button className="submit-button question-palette__submit" onClick={onSubmit} type="button">Submit Test</button></aside>;
}
